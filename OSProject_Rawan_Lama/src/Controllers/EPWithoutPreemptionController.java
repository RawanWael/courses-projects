...
import static Algorithms.EPWithoutPreemption.processListEPWithoutPreemption;
...
import static Algorithms.EPWithoutPreemption.ganttChatList;

public class EPWithoutPreemptionController implements Initializable {
    private ObservableList<Process> dataList;

    @FXML
    private TableView<Process> tvEPWithoutPreemption;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // TODO
        showTable(processListEPWithoutPreemption);
        calculateAverages(processListEPWithoutPreemption);
        showGanttChart(ganttChatList);
    ...
    public void showTable(ArrayList temp) {
        ...
        tvEPWithoutPreemption.setItems(dataList);
    }
    ...
    public void showGanttChart(ArrayList<GanttchartNode> temp) {
        ...
        for (int i = 0; i < temp.size(); i++) {
            if (i == 0 || flag || temp.get(i).getStart() - temp.get(i - 1).getEnd() == 0) {
                double width = (temp.get(i).getEnd() - temp.get(i).getStart()) * mulNum / duration; //where 770 is the total width of the used HBox
                ...
                Label p = new Label("P " + temp.get(i).getId());
                ...
                double width = (temp.get(i).getStart() - temp.get(i - 1).getEnd()) * mulNum / duration;
                Rectangle r = new Rectangle();
                r.setHeight(height);
                r.setWidth(width);
